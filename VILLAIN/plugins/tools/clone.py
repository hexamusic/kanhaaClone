...
from VILLAIN.utils.database import get_assistant
from config import API_ID, API_HASH
from VILLAIN import app
from config import OWNER_ID
from VILLAIN.misc import SUDOERS
from VILLAIN.utils.database import get_assistant, clonebotdb
from VILLAIN.utils.database.clonedb import has_user_cloned_any_bot
from config import LOGGER_ID, CLONE_LOGGER
import requests
from VILLAIN.utils.decorators.language import language
import pyrogram.errors

from VILLAIN.utils.database.clonedb import get_owner_id_from_db
    ...
    if len(message.command) > 1:
        ...
        try:
            ai = Client(
                ...
                API_HASH,
                bot_token=bot_token,
                plugins=dict(root="VILLAIN.cplugin"), 
            )
            await ai.start()
            bot = await ai.get_me()
        ...
        try:
            ...
            await mi.edit_text(
                f"⚠️ <b>ᴇʀʀᴏʀ:</b>\n\n<code>{e}</code>\n\n**ᴋɪɴᴅʟʏ ғᴏᴡᴀʀᴅ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴛᴏ @oldskoolgc ᴛᴏ ɢᴇᴛ ᴀssɪsᴛᴀɴᴄᴇ**"
            )
    else:
